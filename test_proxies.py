...
def test_proxy(proxy):
    try:
        ...
        print(f"  {proxy.split('@')[1][:20]}... → {r.status_code}")
        return r.status_code == 200
    except Exception as e:
        print(f"  ❌ {proxy.split('@')[1][:20]}... → {str(e)[:50]}")
        return False

def main():
    print("=" * 60)
    print("Проверка доступа к Rutube через Webshare прокси")
    print("=" * 60)
    success = 0
    for idx, proxy in enumerate(PROXIES, 1):
        print(f"\n🔎 Прокси {idx}/{len(PROXIES)}")
        if test_proxy(proxy):
            success += 1
        time.sleep(2)
    print(f"\n✅ Работает: {success}/{len(PROXIES)}")
